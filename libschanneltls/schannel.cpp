                                   ...
                                   int sockindex)
{
  ...
  if(data->set.ssl.primary.clientcert || data->set.ssl.primary.cert_blob) {
    ...
    if(blob) {
      ...
      cert_path = curlx_convert_UTF8_to_wchar(
  ...
  if(curlx_verify_windows_version(10, 0, 17763, PLATFORM_WINNT,
    ...
    CRYPTO_SETTINGS crypto_settings[4] = { TlsParametersCngAlgUsageKeyExchange };
  ...
  if(sspi_status != SEC_E_OK) {
    failf(data, "schannel: AcquireCredentialsHandle failed: %#X",
          sspi_status);
    free(backend->cred);
    switch(sspi_status) {
                       ...
                       int sockindex)
{
  ...
  */
  ...
  if(sspi_status != SEC_I_CONTINUE_NEEDED) {
    ...
    switch(sspi_status) {
    case SEC_E_INSUFFICIENT_MEMORY:
      failf(data, "schannel: initial InitializeSecurityContext failed: %#X",
            sspi_status);
      return CURLE_OUT_OF_MEMORY;
    case SEC_E_WRONG_PRINCIPAL:
      failf(data, "schannel: SNI or certificate check failed: %#X",
            sspi_status);
                       ...
                       int sockindex)
{
  ...
  if(!backend->decdata_buffer) {
    ...
    backend->decdata_buffer = (unsigned char*)malloc(backend->decdata_length);
    if(!backend->decdata_buffer) {
      failf(data, "schannel: unable to allocate memory");
      return CURLE_OUT_OF_MEMORY;
    }
  ...
  if(!backend->encdata_buffer) {
    ...
    backend->encdata_buffer = (unsigned char*)malloc(backend->encdata_length);
    if(!backend->encdata_buffer) {
      failf(data, "schannel: unable to allocate memory");
      return CURLE_OUT_OF_MEMORY;
    }
  ...
  if(backend->encdata_length - backend->encdata_offset <
    ...
    size_t reallocated_length = backend->encdata_offset +
      CURL_SCHANNEL_BUFFER_FREE_SIZE;
    reallocated_buffer = (unsigned char*)realloc(backend->encdata_buffer,
                                 reallocated_length);
    ...
    if(sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE ||
      ...
      if(inbuf[1].BufferType == SECBUFFER_DATA) {
        ...
        if(backend->decdata_length - backend->decdata_offset < size ||
          ...
          reallocated_buffer = (unsigned char*)realloc(backend->decdata_buffer,
                                       reallocated_length);
          if(!reallocated_buffer) {
            *err = CURLE_OUT_OF_MEMORY;
            failf(data, "schannel: unable to re-allocate memory");
      ...
      *err = CURLE_RECV_ERROR;
      infof(data, "schannel: failed to read data from server: %#X",
            sspi_status);
      goto cleanup;
    }
  }
...
static void schannel_session_free(void *ptr)
{
  ...
  struct Curl_schannel_cred *cred = (struct Curl_schannel_cred *)ptr;

  if(cred) {
    cred->refcount--;
    if(cred->refcount == 0) {
      s_pSecFn->FreeCredentialsHandle(&cred->cred_handle);
      free(cred->sni_hostname);
#ifdef HAS_CLIENT_CERT_PATH
                             ...
                             int sockindex)
{
  ...
  if(connssl->use && backend->cred && backend->ctxt) {
    ...
    if(sspi_status != SEC_E_OK) {
      failf(data, "schannel: ApplyControlToken failure: %#X",
            sspi_status);
    }
    ...
    if((sspi_status == SEC_E_OK) || (sspi_status == SEC_I_CONTEXT_EXPIRED)) {
      ...
      s_pSecFn->FreeContextBuffer(outbuf.pvBuffer);
      if((result != CURLE_OK) || (outbuf.cbBuffer != (size_t) written)) {
        infof(data, "schannel: failed to send close msg: %d"
                                    ...
                                    const char *pinnedpubkey)
{
  ...
  do {
    ...
    if((sspi_status != SEC_E_OK) || !pCertContextServer) {
      failf(data, "schannel: Failed to read remote certificate context: %#X",
            sspi_status);
      break; /* failed */
    }
